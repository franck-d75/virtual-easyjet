import { deleteAdminHub, updateAdminHub } from "@/lib/api/admin";
...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    ...
    const payload = (await request.json()) as Partial<AdminHubPayload>;
    const result = await executeWithBackendAccess((accessToken) =>
      updateAdminHub(accessToken, id, payload),
    );

    return createBackendJsonResponse(result.data, result.refreshedSession);
  } catch (error) {
    logWebError("admin hub update failed", error);
...
export async function DELETE(_: Request, context: RouteContext) {
  try {
    const { id } = await context.params;
    const result = await executeWithBackendAccess((accessToken) =>
      deleteAdminHub(accessToken, id),
    );

    return createBackendJsonResponse(result.data, result.refreshedSession);
  } catch (error) {
    logWebError("admin hub delete failed", error);
    return createBackendErrorResponse(error);
  }
}
